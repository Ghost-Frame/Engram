...
enum Commands {
    ...
    ///
    /// Examples:
    ///   cred exec kleos claude-code --env EIDOLON_KEY -- \
...
async fn cmd_piv(cmd: PivCmd) -> Result<()> {
    ...
    match cmd {
        PivCmd::Setup { touch_policy } => {
            ...
            for (slot, subject) in [
                (PivSlot::KeyManagement, "CN=credd-ecdh-9d,O=Kleos"),
                (PivSlot::Authentication, "CN=credd-auth-9a,O=Kleos"),
            ] {
...
mod bootstrap_tests {
    ...
    fn cbv1_format_roundtrip() {
        ...
        let header = serde_json::json!({"v":1,"slot":"kleos/credd-test","host":"test"});
        ...
        assert_eq!(hdr["v"], 1);
        assert_eq!(hdr["slot"], "kleos/credd-test");
...
mod user_id_migration_tests {
    ...
    async fn migration_promotes_uid0_rows_when_no_collision() {
        ...
        insert_row(&db, 1, "kleos", "claude-code-host").await; // unrelated row
