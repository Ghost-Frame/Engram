...
/// Filename for the persisted challenge under the kleos config dir.
...
/// Load or create the persistent 32-byte kleos challenge.
///
/// Stored under `~/.config/kleos/challenge` (or `$XDG_CONFIG_HOME/kleos/
...
pub fn get_or_create_challenge() -> Result<[u8; CHALLENGE_SIZE]> {
    ...
    info!("generated new kleos challenge at {}", path.display());
    Ok(challenge)
}
...
/// Config directory for kleos: `$XDG_CONFIG_HOME/kleos` or
/// `~/.config/kleos`.
