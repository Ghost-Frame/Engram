...
mod tests {
    ...
    async fn test_vacuum_into_creates_backup_file() {
        let src_path = std::env::temp_dir().join(format!("kleos-src-{}.db", uuid::Uuid::new_v4()));
        let dst_path = std::env::temp_dir().join(format!("kleos-dst-{}.db", uuid::Uuid::new_v4()));
    ...
    async fn test_integrity_check_passes_for_minimal_db() {
        let path = std::env::temp_dir().join(format!("kleos-minimal-{}.db", uuid::Uuid::new_v4()));
    ...
    async fn test_restore_test_reports_tables_and_schema_version() {
        let path = std::env::temp_dir().join(format!("kleos-restore-{}.db", uuid::Uuid::new_v4()));
        {
    ...
    async fn test_restore_test_errors_on_missing_file() {
        let path = std::env::temp_dir().join(format!("kleos-missing-{}.db", uuid::Uuid::new_v4()));
    ...
    async fn test_restore_test_handles_db_without_memories_table() {
        let path = std::env::temp_dir().join(format!("kleos-no-mem-{}.db", uuid::Uuid::new_v4()));
        {
            let conn = rusqlite::Connection::open(path.to_str().unwrap()).unwrap();
