...
///
/// Backend selection:
/// - `KLEOS_RERANKER_BACKEND=onnx` (default): local ONNX cross-encoder
/// - `KLEOS_RERANKER_BACKEND=http`: remote Cohere/Jina API
/// - `KLEOS_RERANKER_BACKEND=none` or `reranker_enabled=false`: returns None
///
/// For HTTP backend, set:
/// - `KLEOS_RERANKER_HTTP_ENDPOINT` (required, e.g. https://api.cohere.ai/v1/rerank)
/// - `KLEOS_RERANKER_HTTP_API_KEY` (optional, for authenticated APIs)
/// - `KLEOS_RERANKER_HTTP_MODEL` (default: "rerank-v3.5")
///
...
pub async fn create_reranker(
    ...
    match backend.as_str() {
        ...
        "http" | "remote" | "cohere" | "jina" => {
            let endpoint = std::env::var("KLEOS_RERANKER_HTTP_ENDPOINT").map_err(|_| {
                EngError::InvalidInput(
                    "KLEOS_RERANKER_HTTP_ENDPOINT required for http reranker backend".into(),
                )
            })?;
            let api_key = std::env::var("KLEOS_RERANKER_HTTP_API_KEY").ok();
            let model = std::env::var("KLEOS_RERANKER_HTTP_MODEL")
