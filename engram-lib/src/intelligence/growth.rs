...
fn get_prompt_for_service(service: &str, prompt_override: Option<&str>) -> String {
    ...
    match service {
        "kleos" => "You are Kleos's internal self-reflection process. Kleos is a persistent memory system.\n\
        ...
        _ => "You are a self-reflection process for a service in the Kleos ecosystem.\n\
...
/// Self-reflection for Kleos -- called periodically (e.g., every hour).
    ...
    let mut growth_rows = conn
        .query(
            "SELECT content FROM memories \
             WHERE category = 'growth' AND source = 'kleos-growth' AND is_forgotten = 0 AND user_id = ?1 \
...
mod tests {
    ...
    fn test_get_prompt_override() {
        let p = get_prompt_for_service("kleos", Some("Custom prompt"));
