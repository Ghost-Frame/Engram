//! Shared tracing + OpenTelemetry bootstrap for all Kleos binaries.
//!
...
//! `KLEOS_OTLP_ENDPOINT` is set, an OTLP/gRPC span exporter is added and
...
///
/// - `service_name`: OTel `service.name` resource attribute (e.g. `"kleos-server"`).
...
pub fn init_tracing(service_name: &str, default_filter: &str) -> OtelGuard {
    ...
    let otlp_endpoint = std::env::var("OTEL_EXPORTER_OTLP_ENDPOINT")
        ...
        .filter(|s| !s.is_empty())
        .or_else(|| {
            std::env::var("KLEOS_OTLP_ENDPOINT")
                .ok()
                .filter(|s| !s.is_empty())
        });
