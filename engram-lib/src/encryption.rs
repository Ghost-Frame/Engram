...
//!
//! **YubiKey mode** is intentionally not handled here because kleos-cred
//! depends on kleos-lib (circular dep). Binaries that need yubikey mode
...
/// the key at the binary level via kleos-cred.
...
/// Read a raw 32-byte key from `$XDG_CONFIG_HOME/kleos/dbkey` (or
/// `~/.config/kleos/dbkey`). Rejects files with group/world-readable
/// permissions on Unix.
...
/// Keyfile path: `$XDG_CONFIG_HOME/kleos/dbkey` or `~/.config/kleos/dbkey`.
...
/// Read `KLEOS_DB_KEY` env var and hex-decode to 32 bytes.
fn resolve_env() -> Result<[u8; KEY_SIZE]> {
    let hex_str = SecretString::new(std::env::var("KLEOS_DB_KEY").map_err(|_| {
        EngError::Encryption(
            "KLEOS_DB_KEY environment variable not set (encryption.mode = env requires it)".into(),
        )
    })?);

    let mut bytes = hex::decode(hex_str.expose_secret().trim()).map_err(|e| {
        EngError::Encryption(format!(
            "KLEOS_DB_KEY is not valid hex: {} -- expected 64 hex characters (32 bytes)",
            e
        ))
    })?;
    ...
    if bytes.len() != KEY_SIZE {
        return Err(EngError::Encryption(format!(
            "KLEOS_DB_KEY is {} bytes, expected {} (64 hex chars)",
            bytes.len(),
            KEY_SIZE,
        )));
    }
...
mod tests {
    ...
    fn resolve_env_missing_var_fails() {
        ...
        std::env::remove_var("KLEOS_DB_KEY");
    ...
    fn resolve_env_bad_hex_fails() {
        std::env::set_var("KLEOS_DB_KEY", "not-hex-at-all");
        let result = resolve_env();
        assert!(result.is_err());
    ...
    fn resolve_env_wrong_length_fails() {
        ...
        std::env::set_var("KLEOS_DB_KEY", "aa".repeat(16));
        let result = resolve_env();
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
    ...
    fn resolve_env_valid_key() {
        ...
        std::env::set_var("KLEOS_DB_KEY", &key_hex);
    ...
    fn resolve_keyfile_missing_file_fails() {
        ...
        let dir = std::env::temp_dir().join(format!("kleos-test-{}", uuid::Uuid::new_v4()));
    ...
    fn resolve_keyfile_wrong_size_fails() {
        let dir = std::env::temp_dir().join(format!("kleos-test-{}", uuid::Uuid::new_v4()));
        let kleos_dir = dir.join("kleos");
        std::fs::create_dir_all(&kleos_dir).unwrap();
        let path = kleos_dir.join("dbkey");
