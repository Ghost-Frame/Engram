use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Config {
    pub db_path: String,
    pub host: String,
    pub port: u16,
    pub api_key: Option<String>,
    pub embedding_dim: usize,
    pub default_retention: f32,
    pub embedding_model: String,
    pub embedding_max_seq: usize,
    pub embedding_model_dir: Option<String>,
    pub embedding_onnx_file: String,
    pub embedding_chunk_max_chars: usize,
    pub embedding_chunk_overlap: usize,
    pub embedding_chunk_max_chunks: usize,
    pub reranker_enabled: bool,
    pub reranker_top_k: usize,
}

impl Default for Config {
    fn default() -> Self {
        Self {
            db_path: "kleos.db".to_string(),
            host: "127.0.0.1".to_string(),
            port: 4200,
            api_key: None,
            embedding_dim: 1024,
            default_retention: 0.9,
            embedding_model: "BAAI/bge-m3".to_string(),
            embedding_max_seq: 512,
            embedding_model_dir: None,
            embedding_onnx_file: "model_quantized.onnx".to_string(),
            embedding_chunk_max_chars: 1440,
            embedding_chunk_overlap: 160,
            embedding_chunk_max_chunks: 6,
            reranker_enabled: true,
            reranker_top_k: 12,
        }
    }
}

/// Eidolon-specific configuration -- gate, growth, sessions, prompt settings.
/// Kept isolated from the core Kleos config.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EidolonConfig {
    pub gate: GateConfig,
    pub growth: GrowthConfig,
    pub sessions: SessionsConfig,
    pub prompt: PromptConfig,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GateConfig {
    /// Commands that are always blocked, in addition to built-in patterns.
    #[serde(default)]
    pub extra_blocked_patterns: Vec<String>,
    /// Server names that should never be targeted in SSH commands.
    #[serde(default)]
    pub reserved_targets: Vec<String>,
    /// Seconds to wait for human approval before auto-denying.
    #[serde(default = "default_gate_approval_timeout")]
    pub approval_timeout_secs: u64,
}

fn default_gate_approval_timeout() -> u64 { 120 }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GrowthConfig {
    /// How often to trigger automatic growth reflection, in seconds (0 = disabled).
    #[serde(default)]
    pub reflection_interval_secs: u64,
    /// Maximum number of observations to store.
    #[serde(default = "default_observation_limit")]
    pub observation_limit: usize,
}

fn default_observation_limit() -> usize { 1000 }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SessionsConfig {
    /// Maximum number of concurrent active sessions.
    #[serde(default = "default_max_concurrent_sessions")]
    pub max_concurrent: usize,
    /// Maximum output buffer size per session (lines).
    #[serde(default = "default_session_buffer_size")]
    pub buffer_size: usize,
    /// Seconds before an inactive streaming connection is dropped.
    #[serde(default = "default_stream_timeout")]
    pub stream_timeout_secs: u64,
}

fn default_max_concurrent_sessions() -> usize { 50 }
fn default_session_buffer_size() -> usize { 10_000 }
fn default_stream_timeout() -> u64 { 300 }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PromptConfig {
    /// Default max tokens for prompt generation.
    #[serde(default = "default_prompt_max_tokens")]
    pub default_max_tokens: usize,
    /// Weight for personality memories vs. regular memories (0.0-1.0).
    #[serde(default = "default_personality_weight")]
    pub personality_weight: f64,
}

fn default_prompt_max_tokens() -> usize { 4000 }
fn default_personality_weight() -> f64 { 0.3 }

impl Default for EidolonConfig {
    fn default() -> Self {
        EidolonConfig {
            gate: GateConfig {
                extra_blocked_patterns: vec![],
                reserved_targets: vec![],
                approval_timeout_secs: default_gate_approval_timeout(),
            },
            growth: GrowthConfig {
                reflection_interval_secs: 0,
                observation_limit: default_observation_limit(),
            },
            sessions: SessionsConfig {
                max_concurrent: default_max_concurrent_sessions(),
                buffer_size: default_session_buffer_size(),
                stream_timeout_secs: default_stream_timeout(),
            },
            prompt: PromptConfig {
                default_max_tokens: default_prompt_max_tokens(),
                personality_weight: default_personality_weight(),
            },
        }
    }
}

impl Config {
    pub fn from_env() -> Self {
        let mut config = Self::default();
        if let Ok(v) = std::env::var("KLEOS_DB_PATH") {
            config.db_path = v;
        }
        if let Ok(v) = std::env::var("KLEOS_HOST") {
            config.host = v;
        }
        if let Ok(v) = std::env::var("KLEOS_PORT") {
            if let Ok(p) = v.parse() {
                config.port = p;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_API_KEY") {
            config.api_key = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_DIM") {
            if let Ok(d) = v.parse() {
                config.embedding_dim = d;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_DEFAULT_RETENTION") {
            if let Ok(r) = v.parse() {
                config.default_retention = r;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MODEL") {
            config.embedding_model = v;
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MAX_SEQ") {
            if let Ok(n) = v.parse() { config.embedding_max_seq = n; }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MODEL_DIR") {
            config.embedding_model_dir = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_ONNX_MODEL_FILE") {
            config.embedding_onnx_file = v;
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_MAX_CHARS") {
            if let Ok(n) = v.parse() { config.embedding_chunk_max_chars = n; }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_OVERLAP") {
            if let Ok(n) = v.parse() { config.embedding_chunk_overlap = n; }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_MAX_CHUNKS") {
            if let Ok(n) = v.parse() { config.embedding_chunk_max_chunks = n; }
        }
        if let Ok(v) = std::env::var("KLEOS_CROSS_ENCODER") {
            config.reranker_enabled = v != "0";
        }
        if let Ok(v) = std::env::var("KLEOS_RERANKER_TOP_K") {
            if let Ok(n) = v.parse() { config.reranker_top_k = n; }
        }
        config
    }

    /// Returns the resolved model directory for a given model name.
    /// Priority: KLEOS_EMBEDDING_MODEL_DIR env > data_dir/kleos/models/<model_short_name>
    pub fn model_dir(&self, model_short_name: &str) -> std::path::PathBuf {
        if let Some(ref dir) = self.embedding_model_dir {
            std::path::PathBuf::from(dir)
        } else {
            
            dirs::data_dir()
                .unwrap_or_else(|| std::path::PathBuf::from("."))
                .join("kleos")
                .join("models")
                .join(model_short_name)
        }
    }
}
