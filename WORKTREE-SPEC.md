...
## Worktree Path
`~/Projects/kleos-wt-J` -- branch `feat/contract-tests`

## Goal
Add parity tests that verify the Rust server produces the same API responses as the TypeScript kleos. Currently there are ZERO contract tests.

## Source of Truth
- `C:\Users\Zan\Projects\kleos\tests\api.test.mjs` -- TS API test suite (33 tests, 14 suites)
- `C:\Users\Zan\Projects\kleos\server.ts` -- TS server for endpoint reference

## Tasks

### 1. Read TS Test Suite First
Read `C:\Users\Zan\Projects\kleos\tests\api.test.mjs` to understand what's tested and what the expected response shapes are.

### 2. Create Server Integration Tests
- Create: `kleos-server/tests/api_parity.rs` (or similar)
...
### 3. Add Fixture-Based Tests in kleos-lib
