...
- Bootstrap requires a pre-shared `KLEOS_BOOTSTRAP_SECRET` and is one-time only (atomic claim via database sentinel)
...
- HuggingFace model downloads can be disabled with `KLEOS_EMBEDDING_OFFLINE_ONLY=1`

## Security Hardening Checklist

For production deployments:

- [ ] Set `KLEOS_API_KEY` to a strong, unique value
- [ ] Bind to localhost and use a reverse proxy (nginx, Caddy) for TLS
- [ ] Set `KLEOS_EMBEDDING_OFFLINE_ONLY=1` if running air-gapped
- [ ] Configure `KLEOS_EIDOLON_GATE_BLOCKED_PATTERNS` for dangerous commands
...
- GUI password (`KLEOS_GUI_PASSWORD`) is checked via timing-safe comparison but transmitted in plaintext without TLS
...
**kleos-migrate only** (via libsql 0.6.0 -- does not affect server, CLI, or credd):
...
`kleos-migrate` has a linker conflict between libsqlite3-sys (SQLCipher) and libsql-ffi (bundled sqlite3). Both define the same sqlite3 symbols. This is a one-shot ETL utility and does not affect the server, CLI, or credential daemon.
