...
See `~/Documents/specs/2026-04-10-kleos-scalability-phase3-5.md` section "Phase 5".
...
- `kleos-lib/src/tenant/mod.rs` -- registry, handle, config types
- `kleos-lib/src/tenant/registry_db.rs` -- system/registry.db access
- `kleos-lib/src/tenant/loader.rs` -- lazy load, LRU eviction
- `kleos-lib/src/tenant/id.rs` -- `tenant_id_from_user` helper with safe fallback hashing
- `kleos-cli/src/bin/kleos-migrate.rs` -- monolithic-to-sharded migration tool
- `kleos-cli/src/migrate/mod.rs` -- migration tool library code (shared with bin)
- `kleos-cli/src/migrate/libsql_reader.rs` -- libsql read side
- `kleos-cli/src/migrate/rusqlite_writer.rs` -- rusqlite + LanceDB write side
- `kleos-cli/src/migrate/verify.rs` -- post-migration verification

## Cargo Dependency Policy

- `kleos-lib`: rusqlite only, no libsql
- `kleos-server`: no libsql
- `kleos-cli`: BOTH libsql (for migration) and rusqlite (via kleos-lib). This is the only crate that links both.

This is enforced by a workspace check in Phase 4's final commit.

## Files to Modify

- `kleos-server/src/state.rs` -- `AppState` holds `Arc<TenantRegistry>` instead of `Arc<Database>`
- Every HTTP handler under `kleos-server/src/routes/` -- resolve tenant from auth before any db call
- Every per-tenant table definition in `kleos-lib/src/db/schema.rs` -- drop `user_id` column
- Every per-tenant SQL query -- drop `WHERE user_id = ?` clause
- `kleos-lib/src/jobs/*` -- iterate tenants via registry, cap concurrency
- `kleos-server/src/routes/admin/*` -- add tenant lifecycle endpoints, use registry
...
Phase 4 drops libsql from `kleos-lib` and `kleos-server`, but existing deployments have libsql-format databases containing the `memories_vec_1024_idx` virtual table from prior schema versions. Vanilla rusqlite cannot open a schema that references libsql-specific virtual tables without errors. The migration tool uses:
...
`kleos-cli/Cargo.toml` keeps libsql as a dependency. After a release or two, once users have migrated, a cleanup commit can remove libsql entirely.

### Binary Location

```
kleos-cli/src/bin/kleos-migrate.rs
```

### Algorithm

1. **Lock**: write `<data_dir>/.migration-in-progress` lockfile. Fail fast if it already exists.
2. **Read old DB**: open `kleos.db` via libsql (read-only). Verify schema version and report it.
...
4. **For each user_id**:
   ...
   c. Create a fresh `kleos.db` in that directory via rusqlite with the NEW schema (no `user_id` column, no `memories_vec_1024_idx` virtual table, includes Phase 3 `memory_pagerank` tables).
...
7. **Swap**: rename old `kleos.db` to `kleos.db.pre-shard-backup`. Do NOT delete.
8. **Unlock**: remove `.migration-in-progress` lockfile.
9. **Summary**: print tenant count, row counts, time taken, disk used.

### Modes

- `kleos-migrate --data-dir <dir> --dry-run`
  - Reports distinct user_id count, per-tenant row counts, estimated disk use. Writes nothing.
- `kleos-migrate --data-dir <dir>`
  - Full migration. Requires server to be stopped.
- `kleos-migrate --data-dir <dir> --verify`
  - After a successful migration, reruns counts and checksums as a sanity pass.
- `kleos-migrate --data-dir <dir> --reverse`
  - Consolidates `data_dir/tenants/*` back into a single rusqlite `kleos.db` at the root. Rolls back the sharding transformation. Note: the rollback target is vanilla SQLite, NOT libsql, because we are not regressing drivers. Rollback does not restore libsql-specific virtual tables.
...
- Migration: run `kleos-migrate --dry-run` and `kleos-migrate` on a fixture monolithic DB, verify row counts and schemas.
...
- `kleos-migrate --reverse` consolidates shards back.
...
- **Backup tooling**: existing backup scripts assume one DB. Rewrite to walk `tenants/` directory or provide `kleos-backup` helper.
...
- [ ] `kleos-migrate` converts a monolithic DB to sharded layout
- [ ] `kleos-migrate --dry-run` reports correctly without writing
...
- [ ] `kleos-migrate --dry-run` runs cleanly on a libsql fixture
- [ ] `kleos-migrate` successfully converts a libsql fixture with vector virtual tables
...
- [ ] `kleos-migrate --reverse` consolidates shards back to vanilla SQLite
- [ ] `kleos-migrate --resume` picks up after a partial failure
- [ ] Lockfile prevents concurrent runs
- [ ] libsql is ONLY referenced from the kleos-cli crate, not kleos-lib or kleos-server
...
tables so missed query rewrites fail loudly. Adds kleos-migrate
