...
- Read ALL files in `kleos-lib/src/intelligence/` to understand existing implementations
- Read `kleos-server/src/routes/intelligence.rs` for existing route patterns
- Read `kleos-server/src/routes/growth.rs` for growth endpoint patterns
- Read the TS source at approximately lines 2120-3900 of `kleos/src/routes/index.ts` for the missing endpoint implementations
...
**Read:** `kleos-lib/src/intelligence/sentiment.rs`

Identify the public functions. Expected: `analyze_sentiment(content: &str) -> SentimentResult` or similar.

**Add routes to `kleos-server/src/routes/intelligence.rs`:**
...
**Read:** `kleos-lib/src/intelligence/valence.rs`
...
**Read:** `kleos-lib/src/intelligence/predictive.rs`
...
**Read:** `kleos-lib/src/intelligence/reconsolidation.rs`
...
**Read:** `kleos-lib/src/intelligence/extraction.rs`
...
**Implement in lib:** `kleos-lib/src/intelligence/temporal.rs` (extend existing file)
...
async fn timetravel_handler(
    ...
    let results = kleos_lib::intelligence::temporal::time_travel(
...
**Implement in lib:** Extend `kleos-lib/src/intelligence/consolidation.rs`
...
**Implement in lib:** `kleos-lib/src/memory/mod.rs` or new file `kleos-lib/src/intelligence/correction.rs`
...
**Implement in lib:** `kleos-lib/src/intelligence/mod.rs` or new file `health.rs`
...
**Implement in lib:** New file `kleos-lib/src/intelligence/feedback.rs`
...
Add this migration to `kleos-lib/src/db/` (check existing migration pattern).
...
**Implement in lib:** New file `kleos-lib/src/intelligence/duplicates.rs`
...
The `/growth/*` routes already exist in `kleos-server/src/routes/growth.rs`. Verify they call into `intelligence/growth.rs` properly. If growth.rs relies on eidolon's LLM reflection, ensure the LLM client (`state.llm`) is passed through.
...
Add to schema initialization (check `kleos-lib/src/db/` for pattern):
...
If you create new files (feedback.rs, duplicates.rs, correction.rs, health.rs), add them to `kleos-lib/src/intelligence/mod.rs`:
...
Update the router in `kleos-server/src/routes/intelligence.rs` to include all new routes:
