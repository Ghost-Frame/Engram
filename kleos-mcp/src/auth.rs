...
pub async fn resolve_auth(app: &App, args: &Value) -> Result<AuthContext> {
    let token = resolve_bearer_token(args).ok_or_else(|| {
        EngError::Auth(
            "missing bearer_token argument and KLEOS_MCP_BEARER_TOKEN env var is unset".into(),
        )
    })?;
...
/// `KLEOS_MCP_BEARER_TOKEN` env var (and its `KLEOS_MCP_BEARER_TOKEN`
...
fn resolve_bearer_token(args: &Value) -> Option<String> {
    ...
    if let Ok(token) = std::env::var("KLEOS_MCP_BEARER_TOKEN") {
        if !token.is_empty() {
            return Some(token);
        }
    }
...
mod tests {
    ...
    fn with_env<F: FnOnce()>(set: Option<&str>, body: F) {
        ...
        let prev = std::env::var("KLEOS_MCP_BEARER_TOKEN").ok();
        match set {
    ...
    /// `KLEOS_MCP_BEARER_TOKEN` into `KLEOS_MCP_BEARER_TOKEN`. We invoke it
    ...
    fn kleos_alias_env_via_migrate() {
        ...
        std::env::remove_var("KLEOS_MCP_BEARER_TOKEN");
