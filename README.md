...
---
GUI DEMO: [Demo](https://demo-gui.example.com)
## Why Rust?

Kleos started as a Node.js server. It worked. But a memory system that injects into every agent turn has to stay fast under load, and Node made that harder than it should have been.
...
The TypeScript kleos remains the reference design. This is the runtime.
...
### Developer Platform
- **REST API**: 80+ endpoints, drop-in compatible with the reference kleos
- **Rust CLI**: `kleos-cli` for store, search, context, recall, list, bootstrap
- **MCP Server**: `kleos-mcp` for LLM tool integration via Model Context Protocol (stdio or HTTP)
- **Sidecar**: `kleos-sidecar` for session-scoped agent runs
- **Credential Manager**: `kleos-cred` CLI + `kleos-credd` daemon for encrypted credential vault with YubiKey support
...
Kleos bundles a set of coordination services behind the same auth and database:
...
| Crate | Role |
|-------|------|
| `kleos-lib` | Core library. Memory, search, embeddings, graph, intelligence, services, auth, jobs. |
| `kleos-server` | Axum HTTP server. Binds routes to library functions. Handles middleware, auth, rate limiting, GUI. |
| `kleos-cli` | Command-line client over the HTTP API. |
| `kleos-sidecar` | Session-scoped memory proxy with file watcher and local caching. Talks to `kleos-server`. |
| `kleos-mcp` | MCP (Model Context Protocol) stdio and HTTP entrypoint for LLM tool integration. |
| `kleos-cred` | Credential management CLI with encrypted vault and YubiKey support. |
| `kleos-credd` | Credential management daemon. Serves credentials over local HTTP with session auth. |
| `kleos-approval-tui` | Terminal UI for human approval workflow. Ratatui-based interactive review queue. (WIP) |
| `kleos-migrate` | ETL tool for migrating from libsql to rusqlite + LanceDB. |
...
Every endpoint needs `Authorization: Bearer eg_...` unless the server runs in open-access mode. Shapes match the reference kleos: OpenAPI clients, SDKs, and MCP integrations work without changes.
...
The full endpoint list (inbox, sync, webhooks, digests, pack, prompts, scratchpad, gate, grounding, growth, sessions, agents, artifacts, portability, onboarding, docs) lives in `kleos-server/src/server.rs` and `kleos-server/src/routes/`.
...
kleos-cli recall 42
kleos-cli guard "rm -rf ./data"
kleos-cli list --limit 20
kleos-cli delete 42
kleos-cli bootstrap
```

Every command takes `--server` and `--key` overrides, or reads `KLEOS_URL` / `KLEOS_API_KEY` from the environment.
...
kleos-sidecar --session-id my-session --watch
```

The sidecar is a lightweight HTTP service that talks to `kleos-server` for storage and search. It doesn't run embeddings locally.
...
`kleos_lib::config::Config` reads environment variables. Defaults live in `kleos-lib/src/config.rs`.
...
| `KLEOS_EMBEDDING_ONNX_FILE` | `model_quantized.onnx` | Model filename inside the model dir |
...
| `KLEOS_PAGERANK_REFRESH_INTERVAL_SECS` | `300` | Worker refresh cadence |
...
| `yubikey` | Derive key from YubiKey HMAC-SHA1 challenge-response. Requires `kleos-credd`. |
...
See `kleos-lib/src/config.rs` for the full set, including decomposition tunables, search floors, and Eidolon integration flags.
...
## Relationship to the Reference Kleos

This repo is the Rust port of the TypeScript kleos. API shapes, database schemas, and behavior stay compatible so existing clients, MCP tools, and data migrate without rewrites. The reference implementation holds the design spec. New features land in Rust first once the runtime catches up.

---

<div align="center">

[kleos.example.com](https://kleos.example.com) · [Reference implementation (GitHub)](https://github.com/Ghost-Frame/kleos-ts)
