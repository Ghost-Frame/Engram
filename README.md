...
| `kleos-sidecar` | Session-scoped memory companion for individual agent runs. Embeds `kleos-lib`. |
...
`kleos-sidecar` runs next to a single agent session. It embeds `kleos-lib` and binds to a private port so:

- The main tenant never sees scratch state from in-flight runs
- Every write carries the session ID
- The sidecar shares the libsql database and embedding provider with `kleos-server`
- Long workflows get a zero-latency memory cache

```bash
KLEOS_SIDECAR_PORT=7711 \
KLEOS_SIDECAR_SOURCE=claude-session \
cargo run -p kleos-sidecar -- --session-id my-session
```

Most deployments only need `kleos-server`. The sidecar is there when you want it.
...
| `KLEOS_DB_PATH` | `kleos.db` | libsql database file |
