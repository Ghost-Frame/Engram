...
Thanks for your interest in contributing to Kleos.
...
```
kleos/
  kleos-lib/      -- Core library (all domain logic)
  kleos-server/   -- HTTP API (Axum)
  kleos-cli/      -- CLI client
  kleos-sidecar/  -- Session companion
  kleos-mcp/      -- MCP server
  kleos-cred/     -- Credential types
  kleos-credd/    -- Credential daemon
  kleos-approval-tui/  -- Approval TUI
  kleos-migrate/  -- Migration ETL
  agent-forge/     -- Agent protocol
```

Library code goes in `kleos-lib`. Server routes go in `kleos-server`. Don't put domain logic in the server crate.
