...
//!      [CRED:v3] kleos/<agent> memory row from Kleos, decrypts it with
...
pub async fn get_bootstrap_kleos_bearer(
    ...
    // Fetch the [CRED:v3] kleos/<agent> memory from Kleos, decrypt it
    ...
    let target_prefix = format!("[CRED:v3] kleos/{} = ", params.agent);
    let entry = list
        .results
        .iter()
...
async fn resolve_agent_bearer(state: &AppState, agent: &str) -> Result<String, AppError> {
    ...
    let target_prefix = format!("[CRED:v3] kleos/{} = ", agent);
    let entry = list
        .results
        .iter()
        .find(|m| m.content.starts_with(&target_prefix))
...
pub struct EcdhBearerRequest {
    /// Agent slot (e.g. "claude-code"). Used for HKDF info field.
    pub agent: String,
