...
// KleosServiceClient
//
// Thin async client for the four Kleos sub-systems: Axon, Chiasm, Broca,
...
// errors at trace level so Kleos flakes never block observation ingestion.
// ---------------------------------------------------------------------------

#[derive(Clone)]
pub struct KleosServiceClient {
    http: reqwest::Client,
    base_url: String,
    token: Option<String>,
    pub enabled: bool,
...
impl KleosServiceClient {
    /// Construct from the supplied params. Reads `KLEOS_SIDECAR_KLEOS` to
    ...
    pub fn new_from_env(http: reqwest::Client, base_url: String, token: Option<String>) -> Self {
        let enabled = std::env::var("KLEOS_SIDECAR_KLEOS")
            .map(|v| v.trim() == "1")
            .unwrap_or(false);
        Self {
            http,
    ...
    pub async fn register_soma_agent(
        ...
        let url = match kleos_lib::net::validate_outbound_url(&url_str) {
            Ok(u) => u,
            Err(e) => {
                tracing::trace!(error = %e, "kleos: soma register url invalid");
                return None;
            }
        };
        ...
        match req.send().await {
            Ok(resp) if resp.status().is_success() => {
                ...
                if let Some(id) = id {
                    ...
                    tracing::debug!(agent_id = id, "kleos: soma agent registered");
                }
                id
            }
            Ok(resp) => {
                tracing::trace!(status = %resp.status(), "kleos: soma register non-success");
                None
            }
            Err(e) => {
                tracing::trace!(error = %e, "kleos: soma register failed");
                None
            }
        }
    }
        ...
        tokio::spawn(async move {
            ...
            if let Ok(url) = kleos_lib::net::validate_outbound_url(&hb_url) {
                ...
                if let Err(e) = req.send().await {
                    tracing::trace!(error = %e, "kleos: soma heartbeat ping failed");
                }
            }
            ...
            if let Ok(url) = kleos_lib::net::validate_outbound_url(&log_url) {
                ...
                if let Err(e) = req.send().await {
                    tracing::trace!(error = %e, "kleos: soma heartbeat log failed");
                }
            }
        });
    }
    ...
    pub fn publish_axon(&self, channel: &str, action: &str, payload: Value) {
        ...
        tokio::spawn(async move {
            let result =
                ...
                .await;
            if let Err(e) = result {
                tracing::trace!(error = %e, "kleos: publish_axon failed after retries");
            }
        });
    }
    ...
    pub fn upsert_chiasm_task(&self, session_id: &str, status: &str, summary: &str) {
        ...
        tokio::spawn(async move {
            let result =
                ...
                .await;
            if let Err(e) = result {
                tracing::trace!(error = %e, "kleos: upsert_chiasm_task failed after retries");
            }
        });
    }
    ...
    pub fn log_broca(&self, service: &str, action: &str, narrative: &str) {
        ...
        tokio::spawn(async move {
            let result =
                ...
                .await;
            if let Err(e) = result {
                tracing::trace!(error = %e, "kleos: log_broca failed after retries");
            }
        });
    }
}
