...
pub async fn flush_pending(state: &SidecarState, session_id: &str) -> usize {
    ...
    match result {
        Ok(Some(flags)) => {
            ...
            if n > 0 {
                fire_kleos_flush_hooks(state, session_id, n, &successful);
            }
...
/// Fire Kleos hooks after a successful batch flush. Separated so both the
/// primary batch path and the per-observation fallback path share the same calls.
fn fire_kleos_flush_hooks(
    state: &SidecarState,
    session_id: &str,
    count: usize,
