...
async fn main() {
    ...
    let kleos_client = Arc::new(kleos::KleosServiceClient::new_from_env(
        client.clone(),
        rc.kleos_url.clone(),
    ...
    // Soma registration and 60s heartbeat (opt-in via KLEOS_SIDECAR_KLEOS=1).
    if state.kleos.enabled {
        state
            .kleos
            .register_soma_agent(
                "kleos-sidecar",
        ...
        let hb_kleos = state.kleos.clone();
