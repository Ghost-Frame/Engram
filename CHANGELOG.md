...
- `/chiasm/*` route aliases for Kleos-convention task endpoints
...
- R8 hardening: `validate_outbound_url` gate on sidecar's `kleos_url`; non-unix `tighten_secret_perms` emits a warning; v1 API key acceptance emits counter + warning; webhook `deliver_with_retry` wrapped in panic-catching spawn with counters
...
- Gate hardening: brain-grounded check via Hopfield recall, DNS rebind/SSRF defense, agent allowlist, kleos_stores enforcement
- `cognithor` module renamed to `cognitive`

## [0.3.2] - 2026-04-14

Full rename from Kleos to Kleos.

### Changed

- All 10 workspace crates renamed `kleos-*` to `kleos-*`
- Internal Rust identifiers swept from `kleos_*` to `kleos_*`
- Binary names changed to `kleos-server`, `kleos-cli`, etc. with `kleos-*` symlink aliases
- `KLEOS_*` environment variable prefix with `KLEOS_*` fallback via `migrate_env_prefix()`
- Default database path changed to `kleos.db` with `kleos.db` fallback
- Python SDK package renamed `kleos-client` to `kleos-client`; Go SDK import path updated
...
- Scoped crates.io publishing to `kleos-lib` and `kleos-cred` only (three other names held by third parties)
...
- `kleos-mcp` added to release artifact collection

## [0.1.0] - 2026-04-13

Initial release. Ground-up Rust rewrite of the [TypeScript Kleos](https://github.com/Ghost-Frame/Kleos).
...
- `kleos-cli`, `kleos-mcp`, `kleos-sidecar`, `kleos-cred`/`kleos-credd`
