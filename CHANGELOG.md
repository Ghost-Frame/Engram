...
- **CLI**: `kleos-cli` for store, search, context, recall, list, bootstrap, guard
- **MCP server**: `kleos-mcp` for LLM tool integration via Model Context Protocol (stdio or HTTP)
- **Sidecar**: `kleos-sidecar` for session-scoped memory with file watcher and local caching
- **Credential manager**: `kleos-cred` CLI + `kleos-credd` daemon with encrypted vault and YubiKey support
- **Claude Code hooks**: ready-to-use hooks for session memory, context injection, and tool tracking
- **TypeScript SDK**: `@kleos/sdk` client library
- **Docker support**: multi-stage Dockerfile for containerized deployment
- **GitHub Actions CI**: build, test, clippy, format checks on every push

### Architecture

- 10-crate Cargo workspace: kleos-lib, kleos-server, kleos-cli, kleos-sidecar, kleos-mcp, kleos-cred, kleos-credd, kleos-approval-tui, kleos-migrate, agent-forge
...
[0.1.0]: https://github.com/Ghost-Frame/Kleos-rust/releases/tag/v0.1.0
