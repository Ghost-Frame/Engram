...
///
/// Requires `KLEOS_MCP_BEARER_TOKEN` to be set when using HTTP transport.
...
async fn bearer_auth(request: Request<Body>, next: Next) -> Response {
    let token = match std::env::var("KLEOS_MCP_BEARER_TOKEN") {
...
pub async fn serve(app: App, listen: &str) -> Result<()> {
    ...
    if std::env::var("KLEOS_MCP_BEARER_TOKEN")
        .ok()
        .filter(|s| !s.is_empty())
        .is_none()
    {
