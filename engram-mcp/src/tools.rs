...
pub fn auth_prop() -> Value {
    json!({
        "bearer_token": { "type": "string", "description": "Optional bearer token override. Defaults to KLEOS_MCP_BEARER_TOKEN." }
    })
}
...
pub async fn dispatch(app: &App, name: &str, args: Value) -> Result<Value> {
    match name {
        ...
        _ => Err(kleos_lib::EngError::NotFound(format!("unknown tool: {name}"))),
    }
}
