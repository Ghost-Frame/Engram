...
pub fn register(out: &mut Vec<ToolDef>) {
    out.extend([
        ...
        ToolDef {
            name: "skill.execute",
            description: "Search for relevant skills matching a task and return them as context. Live LLM execution requires the kleos-server; this tool returns matched skill content for use as guidance.",
            input_schema: || with_auth_props(json!({
...
pub async fn skill_execute(app: &App, args: Value) -> Result<Value> {
    ...
    // No LLM wired in kleos-mcp; return matched skills as context for the caller to use.
