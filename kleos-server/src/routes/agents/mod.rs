...
fn load_or_create_signing_secret() -> String {
    ...
    let path = match signing_secret_path() {
        ...
        None => {
            tracing::error!(
                "no env KLEOS_SIGNING_SECRET_FILE and dirs::data_dir() returned None; \
                 running with an in-memory signing secret (regenerates on every restart). \
                 Set KLEOS_SIGNING_SECRET_FILE to an absolute path to persist."
            );
            return generated;
        }
    };
...
fn signing_secret_path() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("KLEOS_SIGNING_SECRET_FILE") {
