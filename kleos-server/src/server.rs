...
/// Build a CORS layer from the `KLEOS_ALLOWED_ORIGINS` env var (comma
...
fn build_cors_layer() -> CorsLayer {
    ...
    match std::env::var("KLEOS_ALLOWED_ORIGINS") {
        Ok(raw) => {
            let origins: Vec<HeaderValue> = raw
                ...
                .filter(|s| {
                    if s.eq_ignore_ascii_case("null") {
                        tracing::warn!("KLEOS_ALLOWED_ORIGINS contained 'null'; dropping");
                        return false;
                    }
                    true
                })
                .inspect(|s| {
                    ...
                    {
                        tracing::warn!(
                            "KLEOS_ALLOWED_ORIGINS entry {} is plaintext http:// and not loopback",
                            s
                        );
                    }
                })
            ...
            if origins.is_empty() {
                tracing::warn!(
                    "KLEOS_ALLOWED_ORIGINS set but empty/invalid; CORS will reject all origins"
                );
        ...
        Err(_) => {
            tracing::info!(
                "KLEOS_ALLOWED_ORIGINS not set; CORS restricted (set explicitly to enable cross-origin access)"
            );
            base.allow_origin(AllowOrigin::list(Vec::<HeaderValue>::new()))
        }
    }
