...
//!
//! ```rust
//! if std::env::var("KLEOS_TEST_MODEL").is_err() { return; }
//! ```
//!
...
/// Auth is enabled (KLEOS_OPEN_ACCESS=0) and a bootstrap secret is set.
///
...
pub async fn test_app() -> (Router, AppState) {
    std::env::set_var("KLEOS_OPEN_ACCESS", "0");
    std::env::set_var("KLEOS_BOOTSTRAP_SECRET", "test-bootstrap-secret");
