//! Comprehensive zero-LLM database benchmark for Kleos.
//!
...
//!
//! Usage:
//!   cargo run -p kleos-lib --example db_benchmark --release -- [options]
//!
//! Options:
...
fn usage() -> &'static str {
    "Usage: cargo run -p kleos-lib --example db_benchmark --release -- [options]\n\n\
Options:\n\
  --memories <n>    Number of memories to seed (default: 5000)\n\
  --fanout <n>      Ring links per memory (default: 4)\n\
  --samples <n>     Timed iterations per scenario (default: 20)\n\
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== Kleos Database Benchmark (zero-LLM) ===");
    println!(
