    ...
    let pairs: Vec<(String, String)> = std::env::vars()
        .filter_map(|(k, v)| {
            k.strip_prefix("KLEOS_")
                .map(|suffix| (format!("KLEOS_{}", suffix), v))
        })
        .collect();
...
pub enum EncryptionMode {
    ...
    #[default]
    None,
    /// Read a raw 32-byte key from `~/.config/kleos/dbkey`.
    Keyfile,
    /// Hex-decode the `KLEOS_DB_KEY` env var (64 hex chars = 32 bytes).
    Env,
...
impl EidolonConfig {
    ...
    pub fn apply_env(mut self) -> Self {
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_ENABLED") {
            self.enabled = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_URL") {
            self.url = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_API_KEY") {
        ...
        if let Ok(v) = std::env::var("KLEOS_CREDD_AGENT_KEY_ENV") {
            c.credd.agent_key_env = v;
        }
        if let Ok(v) = std::env::var("KLEOS_CREDD_ALLOW_RAW") {
            c.credd.allow_raw = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_CREDD_CACHE_TTL_SECS") {
            if let Ok(n) = v.parse() {
                c.credd.cache_ttl_secs = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_GATE_APPROVAL_TIMEOUT") {
            if let Ok(n) = v.parse() {
                c.gate.approval_timeout_secs = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_GATE_BLOCKED_PATTERNS") {
            c.gate.blocked_patterns = v.split(',').map(|s| s.trim().to_string()).collect();
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_GATE_RESERVED_TARGETS") {
            c.gate.reserved_targets = v.split(',').map(|s| s.trim().to_string()).collect();
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_GROWTH_INTERVAL") {
            if let Ok(n) = v.parse() {
                c.growth.reflection_interval_secs = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_GROWTH_OBSERVATION_LIMIT") {
            if let Ok(n) = v.parse() {
                c.growth.observation_limit = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_SESSIONS_MAX") {
            if let Ok(n) = v.parse() {
                c.sessions.max_concurrent = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_SESSIONS_BUFFER") {
            if let Ok(n) = v.parse() {
                c.sessions.buffer_size = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_SESSIONS_STREAM_TIMEOUT") {
            if let Ok(n) = v.parse() {
                c.sessions.stream_timeout_secs = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_SESSIONS_SCRUB_SECRETS") {
            c.sessions.scrub_secrets = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_PROMPT_MAX_TOKENS") {
            if let Ok(n) = v.parse() {
                c.prompt.default_max_tokens = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_PROMPT_MAX_TOKENS_CAP") {
            if let Ok(n) = v.parse() {
                c.prompt.max_tokens_cap = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_PROMPT_PERSONALITY_WEIGHT") {
            if let Ok(n) = v.parse() {
                c.prompt.personality_weight = n;
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_PROMPT_INCLUDE_MEMORIES") {
            c.prompt.default_include_memories = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_EIDOLON_PROMPT_INCLUDE_PERSONALITY") {
...
pub struct Config {
    ...
    /// Whether the GUI is enabled. Set via KLEOS_GUI_PASSWORD (any non-empty
...
impl Config {
    ...
    /// 1. `KLEOS_CONFIG_FILE` env var
    /// 2. `./kleos.toml` in the current directory
    /// 3. `$XDG_CONFIG_HOME/kleos/config.toml` (or `~/.config/kleos/config.toml`)
    ///
    /// Returns `None` if no config file is found.
    fn resolve_config_path() -> Option<std::path::PathBuf> {
        if let Ok(p) = std::env::var("KLEOS_CONFIG_FILE") {
        ...
        let cwd_path = std::path::PathBuf::from("kleos.toml");
        if cwd_path.exists() {
            return Some(cwd_path);
        }
        if let Some(cfg_dir) = dirs::config_dir() {
            let path = cfg_dir.join("kleos").join("config.toml");
            if path.exists() {
                return Some(path);
            }
        }
    ...
    fn apply_env(mut config: Self) -> Self {
        if let Ok(v) = std::env::var("KLEOS_DB_PATH") {
            config.db_path = v;
        }
        if let Ok(v) = std::env::var("KLEOS_HOST") {
            config.host = v;
        }
        if let Ok(v) = std::env::var("KLEOS_PORT") {
            match v.parse() {
                Ok(p) => config.port = p,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_PORT={}, using default {}",
                    v,
                    config.port
                ),
            }
        ...
        } else if let Ok(v) = std::env::var("KLEOS_API_KEY") {
            config.api_key = Some(SecretString::new(v));
        }
        // If neither is set, api_key stays None -- callers invoke the resolver.
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_DIM") {
            match v.parse() {
                Ok(d) => config.embedding_dim = d,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_EMBEDDING_DIM={}, using default {}",
                    v,
                    config.embedding_dim
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_DEFAULT_RETENTION") {
            match v.parse() {
                Ok(r) => config.default_retention = r,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_DEFAULT_RETENTION={}, using default {}",
                    v,
                    config.default_retention
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MODEL") {
            config.embedding_model = v;
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MAX_SEQ") {
            match v.parse() {
                Ok(n) => config.embedding_max_seq = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_EMBEDDING_MAX_SEQ={}, using default {}",
                    v,
                    config.embedding_max_seq
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_MODEL_DIR") {
            config.embedding_model_dir = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_ONNX_MODEL_FILE") {
            config.embedding_onnx_file = v;
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_OFFLINE_ONLY") {
            config.embedding_offline_only = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_MAX_CHARS") {
            match v.parse() {
                Ok(n) => config.embedding_chunk_max_chars = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_EMBEDDING_CHUNK_MAX_CHARS={}, using default {}",
                    v,
                    config.embedding_chunk_max_chars
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_OVERLAP") {
            match v.parse() {
                Ok(n) => config.embedding_chunk_overlap = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_EMBEDDING_CHUNK_OVERLAP={}, using default {}",
                    v,
                    config.embedding_chunk_overlap
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_EMBEDDING_CHUNK_MAX_CHUNKS") {
            match v.parse() {
                Ok(n) => config.embedding_chunk_max_chunks = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_EMBEDDING_CHUNK_MAX_CHUNKS={}, using default {}",
                    v,
                    config.embedding_chunk_max_chunks
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_RERANKER_ENABLED") {
            config.reranker_enabled = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        } else if let Ok(v) = std::env::var("KLEOS_CROSS_ENCODER") {
            config.reranker_enabled = v != "0";
        }
        if let Ok(v) = std::env::var("KLEOS_RERANKER_MODEL_DIR") {
            config.reranker_model_dir = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_RERANKER_TOP_K") {
            match v.parse() {
                Ok(n) => config.reranker_top_k = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_RERANKER_TOP_K={}, using default {}",
                    v,
                    config.reranker_top_k
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_DATA_DIR") {
            config.data_dir = v;
        }
        if let Ok(v) = std::env::var("KLEOS_LANCE_INDEX_PATH") {
            config.lance_index_path = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_VECTOR_DIMENSIONS") {
            match v.parse() {
                Ok(n) => config.vector_dimensions = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_VECTOR_DIMENSIONS={}, using default {}",
                    v,
                    config.vector_dimensions
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_USE_LANCE_INDEX") {
        ...
        if let Ok(v) = std::env::var("KLEOS_GUI_PASSWORD") {
            config.gui_enabled = !v.is_empty();
        }
        if let Ok(v) = std::env::var("KLEOS_GUI_BUILD_DIR") {
            config.gui_build_dir = Some(v);
        }
        if let Ok(v) = std::env::var("KLEOS_PAGERANK_REFRESH_INTERVAL") {
            match v.parse() {
                Ok(n) => config.pagerank_refresh_interval_secs = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_PAGERANK_REFRESH_INTERVAL={}, using default {}",
                    v,
                    config.pagerank_refresh_interval_secs
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_PAGERANK_DIRTY_THRESHOLD") {
            match v.parse() {
                Ok(n) => config.pagerank_dirty_threshold = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_PAGERANK_DIRTY_THRESHOLD={}, using default {}",
                    v,
                    config.pagerank_dirty_threshold
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_PAGERANK_MAX_CONCURRENT") {
            match v.parse() {
                Ok(n) => config.pagerank_max_concurrent = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_PAGERANK_MAX_CONCURRENT={}, using default {}",
                    v,
                    config.pagerank_max_concurrent
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_PAGERANK_ENABLED") {
            config.pagerank_enabled = v != "0" && !v.eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("KLEOS_DREAMER_ENABLED") {
            config.dreamer_enabled = v != "0" && !v.eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("KLEOS_DREAM_INTERVAL_SECS") {
            match v.parse() {
                Ok(n) => config.dream_interval_secs = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_DREAM_INTERVAL_SECS={}, using default {}",
                    v,
                    config.dream_interval_secs
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_DREAM_IDLE_THRESHOLD_SECS") {
            match v.parse() {
                Ok(n) => config.dream_idle_threshold_secs = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_DREAM_IDLE_THRESHOLD_SECS={}, using default {}",
                    v,
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_ENABLED")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_ENABLED"))
        {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_INTERVAL_SECS")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_INTERVAL_SECS"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_MAX_FIXES_PER_TICK")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_MAX_FIXES_PER_TICK"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_MAX_CAPTURES_PER_TICK")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_MAX_CAPTURES_PER_TICK"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_MAX_DERIVES_PER_TICK")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_MAX_DERIVES_PER_TICK"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_FAILURE_THRESHOLD")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_FAILURE_THRESHOLD"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_MIN_EXECUTIONS")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_MIN_EXECUTIONS"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_REFIX_COOLDOWN_SECS")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_REFIX_COOLDOWN_SECS"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_CAPTURE_TAG")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_CAPTURE_TAG"))
        {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
        ...
        if let Ok(v) = std::env::var("KLEOS_SKILL_EVOLUTION_DERIVE_SIMILARITY")
            .or_else(|_| std::env::var("KLEOS_SKILL_EVOLUTION_DERIVE_SIMILARITY"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_WEB_SEARCH_URL")
            .or_else(|_| std::env::var("KLEOS_WEB_SEARCH_URL"))
        {
            if !v.trim().is_empty() {
        ...
        if let Ok(v) = std::env::var("KLEOS_WEB_SEARCH_TIMEOUT_MS")
            .or_else(|_| std::env::var("KLEOS_WEB_SEARCH_TIMEOUT_MS"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_WEB_SEARCH_DEFAULT_LIMIT")
            .or_else(|_| std::env::var("KLEOS_WEB_SEARCH_DEFAULT_LIMIT"))
        {
            match v.parse() {
        ...
        if let Ok(v) = std::env::var("KLEOS_BACKUP_ENABLED") {
            config.backup_enabled = matches!(v.as_str(), "1" | "true" | "TRUE" | "yes");
        }
        if let Ok(v) = std::env::var("KLEOS_BACKUP_INTERVAL_SECS") {
            match v.parse() {
                Ok(n) => config.backup_interval_secs = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_BACKUP_INTERVAL_SECS={}, using default {}",
                    v,
                    config.backup_interval_secs
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_BACKUP_DIR") {
            config.backup_dir = v;
        }
        if let Ok(v) = std::env::var("KLEOS_BACKUP_RETENTION") {
            match v.parse() {
                Ok(n) => config.backup_retention = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_BACKUP_RETENTION={}, using default {}",
                    v,
                    config.backup_retention
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_BACKUP_RETENTION_DAILY") {
            match v.parse() {
                Ok(n) => config.backup_retention_daily = n,
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_BACKUP_RETENTION_DAILY={}, using default {}",
                    v,
                    config.backup_retention_daily
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_AUTH_KEY_ROTATION_GRACE_HOURS") {
            match v.parse() {
                Ok(n) if n > 0 => config.auth_key_rotation_grace_hours = n,
                Ok(_) => tracing::warn!(
                    "KLEOS_AUTH_KEY_ROTATION_GRACE_HOURS must be > 0, using default {}",
                    config.auth_key_rotation_grace_hours
                ),
                Err(_) => tracing::warn!(
                    "invalid env KLEOS_AUTH_KEY_ROTATION_GRACE_HOURS={}, using default {}",
                    v,
                    config.auth_key_rotation_grace_hours
                ),
            }
        }
        if let Ok(v) = std::env::var("KLEOS_ENCRYPTION_MODE") {
            config.encryption.mode = match v.to_ascii_lowercase().as_str() {
                ...
                other => {
                    tracing::warn!("unknown KLEOS_ENCRYPTION_MODE={}, using none", other);
                    EncryptionMode::None
                }
            };
        }
        ...
        if let Ok(v) = std::env::var("KLEOS_TRUSTED_PROXIES") {
    ...
    /// Falls back to `<data_dir>/kleos/models/<model_short_name>`.
    pub fn model_dir(&self, model_short_name: &str) -> std::path::PathBuf {
        ...
        if let Some(ref dir) = self.embedding_model_dir {
            ...
            // This lets ./data/models/bge-m3 resolve
            // ./data/models/granite-reranker for other models.
...
mod tests {
    ...
    fn from_file_parses_partial_toml_and_uses_defaults() {
        let dir = std::env::temp_dir().join(format!("kleos-cfg-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("kleos.toml");
        std::fs::write(
            &path,
            r#"
host = "0.0.0.0"
...
"#,
    ...
    fn from_file_rejects_malformed_toml() {
        let dir = std::env::temp_dir().join(format!("kleos-cfg-bad-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("kleos.toml");
