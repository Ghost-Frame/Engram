...
impl BrainManager {
    pub fn new(data_dir: String) -> Self {
        let backend = std::env::var("KLEOS_BRAIN_BACKEND").unwrap_or_else(|_| "rust".into());
        let exe_suffix = if cfg!(windows) { ".exe" } else { "" };
        let binary_path = if backend == "cpp" {
            std::env::var("KLEOS_BRAIN_CPP_BIN")
                .unwrap_or_else(|_| format!("eidolon-cpp{}", exe_suffix))
        } else {
            std::env::var("KLEOS_BRAIN_RUST_BIN")
...
/// `KLEOS_BRAIN_MODE` environment variable.
///
...
pub async fn create_brain_backend(
    ...
    let mode = std::env::var("KLEOS_BRAIN_MODE").unwrap_or_else(|_| "hopfield".into());
...
pub async fn create_brain_backend(
    ...
    let mode = std::env::var("KLEOS_BRAIN_MODE").unwrap_or_else(|_| "subprocess".into());
    if mode == "none" {
        return None;
    }
    let mgr = BrainManager::new(data_dir.to_string());
...
mod tests {
    ...
    fn test_format_fallback_with_memories() {
        let result = BrainQueryResult {
            activated: vec![BrainMemory {
                id: 42,
                content: "Kleos stores memories in SQLite".into(),
                category: "docs".into(),
                source: "manual".into(),
                importance: 0.8,
                activation: 0.95,
    ...
    fn test_build_user_prompt_empty() {
        ...
        let prompt = build_user_prompt("What is kleos?", &result, None);
        assert!(prompt.contains("QUERY: What is kleos?"));
    ...
    fn test_build_user_prompt_with_context() {
        let result = BrainQueryResult {
            activated: vec![BrainMemory {
                id: 1,
                content: "Kleos is a memory system".into(),
                category: "docs".into(),
                source: "manual".into(),
                importance: 1.0,
                activation: 0.8,
