...
/// `KLEOS_EMBEDDING_MODEL_MAX_BYTES` env var; sane production values stay
...
fn max_download_bytes() -> u64 {
    std::env::var("KLEOS_EMBEDDING_MODEL_MAX_BYTES")
        .ok()
...
/// `KLEOS_EMBEDDING_MODEL_DIR`, which is the correct signal for
...
async fn ensure_file(path: &Path, url: &str, offline_only: bool) -> Result<()> {
    ...
    if offline_only {
        return Err(EngError::Internal(format!(
            "model file {} is missing and KLEOS_EMBEDDING_OFFLINE_ONLY is set; \
             pre-stage the file (source: {}) into KLEOS_EMBEDDING_MODEL_DIR",
            path.display(),
            url
        )));
    }
