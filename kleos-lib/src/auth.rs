...
/// v2 = SHA-256(pepper || raw_key) when KLEOS_API_KEY_PEPPER is set
const HASH_VERSION_LEGACY: i32 = 1;
const HASH_VERSION_PEPPERED: i32 = 2;

/// Cached pepper from KLEOS_API_KEY_PEPPER environment variable.
/// Must be 64 hex characters (32 bytes).
static API_KEY_PEPPER: OnceLock<Option<[u8; 32]>> = OnceLock::new();

fn get_pepper() -> Option<[u8; 32]> {
    *API_KEY_PEPPER.get_or_init(|| {
        std::env::var("KLEOS_API_KEY_PEPPER").ok().and_then(|hex| {
            if hex.len() != 64 {
                tracing::warn!(
                    "KLEOS_API_KEY_PEPPER must be 64 hex chars (32 bytes), got {}",
                    hex.len()
                );
                return None;
            }
            ...
            match bytes {
                ...
                Err(_) => {
                    tracing::warn!("KLEOS_API_KEY_PEPPER contains invalid hex");
                    None
                }
            }
        })
...
/// Uses v2 (peppered) hashing if KLEOS_API_KEY_PEPPER is set.
///
...
fn generate_key() -> Result<(String, String, String, i32)> {
    ...
    let (key_hash, hash_version) = if let Some(hash) = hash_key_v2(&full_key) {
        ...
        // server without setting KLEOS_API_KEY_PEPPER got v1 hashes
        ...
        if !cfg!(debug_assertions) {
            return Err(crate::EngError::Internal(
                "KLEOS_API_KEY_PEPPER must be set to mint API keys in release builds. \
                 Set a 32+ random byte value and restart."
                    .into(),
            ));
        }
        tracing::warn!(
            "KLEOS_API_KEY_PEPPER not set; issuing v1 (unpeppered) key (debug build only)"
        );
            ...
            while let Some(row) = rows.next().map_err(rusqlite_to_eng_error)? {
                ...
                if matches {
                    ...
                    if hash_version != HASH_VERSION_PEPPERED {
                        ...
                        tracing::warn!(
                            key_prefix = %key_prefix,
                            "v1 (unpeppered) api key accepted; configure KLEOS_API_KEY_PEPPER and re-migrate keys"
                        );
                    }
...
mod tests {
    ...
    async fn setup_db() -> Database {
        let db_path = std::env::temp_dir()
            .join(format!("kleos-auth-test-{}.db", uuid::Uuid::new_v4()))
            .to_string_lossy()
            .into_owned();
        let config = Config {
            db_path,
