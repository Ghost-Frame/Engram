...
The original TypeScript kleos shipped a Google OAuth login flow for the web
GUI (`src/auth/google-auth.ts`). kleos deliberately does not port this.
...
2. **Self-host posture.** kleos is deployed by a single operator (or a
...
- `kleos-server/src/extractors.rs` -- bearer extraction
- `kleos-server/src/routes/auth_keys/` -- key lifecycle
- `kleos-server/src/routes/gui/` -- cookie-based session
