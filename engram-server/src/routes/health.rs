...
async fn get_metrics(State(state): State<AppState>) -> Response<Body> {
    ...
    if let Ok(stats) = jobs::get_job_stats(&state.db.conn).await {
        ...
        lines.push(format!("kleos_jobs_total{{status=\"pending\"}} {}", stats.pending));
        lines.push(format!("kleos_jobs_total{{status=\"running\"}} {}", stats.running));
        lines.push(format!("kleos_jobs_total{{status=\"completed\"}} {}", stats.completed));
        lines.push(format!("kleos_jobs_total{{status=\"failed\"}} {}", stats.failed));
        lines.push(String::new());
    }
