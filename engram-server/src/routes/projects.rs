...
async fn create_project(
    Auth(auth): Auth, State(state): State<AppState>, Json(body): Json<kleos_lib::projects::CreateProjectBody>,
) -> Result<Json<Value>, AppError> {
    let name = body.name.as_deref().unwrap_or("").trim();
    if name.is_empty() { return Err(AppError(kleos_lib::EngError::InvalidInput("name is required".into()))); }
    let status = body.status.as_deref().unwrap_or("active");
    let status = if kleos_lib::projects::VALID_PROJECT_STATUSES.contains(&status) { status } else { "active" };
...
async fn list_projects(
    ...
    let projects = kleos_lib::projects::list_projects(&state.db, auth.user_id, q.status.as_deref()).await?;
...
async fn get_project(
    ...
    let project = kleos_lib::projects::get_project(&state.db, id, auth.user_id).await?
        .ok_or_else(|| AppError(kleos_lib::EngError::NotFound("Project not found".into())))?;
    let memory_ids = kleos_lib::projects::get_project_memory_ids(&state.db, id, auth.user_id).await?;
...
async fn update_project(
    Auth(auth): Auth, State(state): State<AppState>, Path(id): Path<i64>, Json(body): Json<kleos_lib::projects::UpdateProjectBody>,
) -> Result<Json<Value>, AppError> {
    let metadata = body.metadata.as_ref().map(|m| serde_json::to_string(m).unwrap_or_default());
...
async fn link_memory(
    ...
    kleos_lib::projects::get_project(&state.db, id, auth.user_id).await?
        .ok_or_else(|| AppError(kleos_lib::EngError::NotFound("Project not found".into())))?;
    kleos_lib::projects::link_memory(&state.db, mid, id).await?;
...
async fn unlink_memory(
    ...
    kleos_lib::projects::get_project(&state.db, id, auth.user_id).await?
        .ok_or_else(|| AppError(kleos_lib::EngError::NotFound("Project not found".into())))?;
    kleos_lib::projects::unlink_memory(&state.db, mid, id).await?;
