...
async fn execute_skills_handler(
    ...
    if task.is_empty() {
        return Err(AppError::from(kleos_lib::EngError::InvalidInput("task is required".into())));
    }

    // Check if LLM is available
    let Some(ref llm) = state.llm else {
        return Err(AppError::from(kleos_lib::EngError::Internal("No LLM configured".into())));
    };
...
async fn upload_skill_handler(
    ...
    if skill_dir.is_empty() {
        return Err(AppError::from(kleos_lib::EngError::InvalidInput("skill_dir is required".into())));
    }
