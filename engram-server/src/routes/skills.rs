...
fn clamp_offset(raw: Option<usize>) -> Result<usize, AppError> {
    ...
    match raw {
        None => Ok(0),
        Some(n) if n > MAX_OFFSET => Err(AppError::from(kleos_lib::EngError::InvalidInput(
...
fn skill_sync_allowlist() -> Vec<std::path::PathBuf> {
    std::env::var("KLEOS_SKILL_SYNC_PATHS")
        .ok()
        .map(|raw| {
            raw.split(':')
                .map(|s| s.trim())
...
async fn sync_skills_handler(
    ...
    if allowlist.is_empty() {
        return Err(AppError(kleos_lib::EngError::InvalidInput(
            "skill sync disabled: set KLEOS_SKILL_SYNC_PATHS to a colon-separated list of allowed roots".into(),
        )));
    }
