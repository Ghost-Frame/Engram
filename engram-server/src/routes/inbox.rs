...
async fn reject(
    ...
    if let Some(reason) = &body.reason {
        let _ = kleos_lib::inbox::set_forget_reason(&state.db, id, reason, auth.user_id).await;
    }
...
async fn bulk_action(
    ...
    for id in &body.ids {
        match body.action.as_str() {
            "approve" => { kleos_lib::inbox::approve_memory(&state.db, *id, auth.user_id).await?; count += 1; }
            "reject" => { kleos_lib::inbox::reject_memory(&state.db, *id, auth.user_id).await?; count += 1; }
            _ => return Err(AppError(kleos_lib::EngError::InvalidInput("action must be approve or reject".into()))),
        }
    }
