...
async fn put_scratch(
    Auth(auth): Auth, State(state): State<AppState>, Json(body): Json<kleos_lib::scratchpad::ScratchPutBody>,
) -> Result<Json<Value>, AppError> {
    ...
    for e in &entries {
        ...
        kleos_lib::scratchpad::upsert_entry(&state.db, auth.user_id, session, agent, model, &e.key, value, ttl).await?;
        stored += 1;
    }
