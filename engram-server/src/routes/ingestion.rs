...
async fn import_bulk(
    ...
    if body.text.is_none() && body.url.is_none() {
        return Err(AppError(kleos_lib::EngError::InvalidInput("Provide text or url parameter".to_string())));
    }
    let input = if let Some(ref text) = body.text {
        if text.trim().is_empty() {
            return Err(AppError(kleos_lib::EngError::InvalidInput("text must be a non-empty string".to_string())));
        }
        text.clone()
    } else {
        return Err(AppError(kleos_lib::EngError::Internal("URL fetching not yet implemented in Rust port".to_string())));
    };
...
async fn import_json(
    ...
    if body.version.is_none() {
        return Err(AppError(kleos_lib::EngError::InvalidInput("Invalid export format: missing version field".to_string())));
    }
    let memories = body.memories.unwrap_or_default();
    if memories.is_empty() {
        return Err(AppError(kleos_lib::EngError::InvalidInput("Invalid export format: missing memories array".to_string())));
    }
...
async fn import_mem0(
    ...
    let arr = memories.as_array().ok_or_else(|| {
        AppError(kleos_lib::EngError::InvalidInput("Expected array of mem0 memories".to_string()))
    })?;
    let mut imported = 0i64;
    for mem in arr {
...
async fn import_supermemory(
    ...
    let items = body.get("documents").or_else(|| body.get("memories")).or_else(|| body.get("data")).cloned()
        ...
        .ok_or_else(|| AppError(kleos_lib::EngError::InvalidInput("Expected documents/memories array".to_string())))?;
    let arr = items.as_array().ok_or_else(|| AppError(kleos_lib::EngError::InvalidInput("Expected array".to_string())))?;
    let mut imported = 0i64;
    let mut skipped = 0i64;
    for item in arr {
...
async fn ingest_text(
    ...
    if body.url.is_none() && body.text.is_none() {
        return Err(AppError(kleos_lib::EngError::InvalidInput("Provide url or text parameter".to_string())));
    }
    let raw_text;
    let ingest_source;
    let title;
