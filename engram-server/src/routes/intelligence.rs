...
async fn sentiment_history_handler(
    ...
    let mut rows = state
        ...
        .await
        .map_err(|e| AppError::from(kleos_lib::EngError::Database(e)))?;

    let mut history = Vec::new();
    while let Some(row) = rows.next().await.map_err(|e| AppError::from(kleos_lib::EngError::Database(e)))? {
        let id: i64 = row.get(0).map_err(|e| AppError::from(kleos_lib::EngError::Database(e)))?;
        let content: String = row.get(1).map_err(|e| AppError::from(kleos_lib::EngError::Database(e)))?;
        let created_at: String = row.get(2).map_err(|e| AppError::from(kleos_lib::EngError::Database(e)))?;
