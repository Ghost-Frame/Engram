...
async fn get_hmac_secret(data_dir: &str) -> String {
    if let Ok(secret) = std::env::var("KLEOS_HMAC_SECRET") {
        return secret;
    }
...
async fn gui_create_memory(
    ...
    if state.config.gui_password.is_some() && !is_gui_authenticated(&state, &headers).await {
        return Err(AppError::from(kleos_lib::EngError::Auth("GUI auth required".into())));
    }

    let content = body.content.trim();
    if content.is_empty() {
        return Err(AppError::from(kleos_lib::EngError::InvalidInput("content is required".into())));
    }
