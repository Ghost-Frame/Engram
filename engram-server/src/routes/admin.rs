...
use kleos_lib::cred::{ProxyRequest, ProxyResponse};
...
async fn count_rows(state: &AppState, sql: &str) -> Result<i64, AppError> {
    ...
    let row = rows
        ...
        .await
        .map_err(kleos_lib::EngError::Database)?
        .ok_or_else(|| kleos_lib::EngError::Internal("missing stats row".into()))?;
    row.get(0)
        .map_err(kleos_lib::EngError::Database)
        .map_err(AppError)
}
...
/// force `KLEOS_BOOTSTRAP_SECRET` at wire speed. This sliding-window
...
async fn bootstrap(
    ...
    // a pre-shared KLEOS_BOOTSTRAP_SECRET, fed either via an Authorization
    // header or the request body. If the env var is unset, bootstrap is
    // disabled entirely and must be performed out-of-band.
    let Ok(expected) = std::env::var("KLEOS_BOOTSTRAP_SECRET") else {
        return Ok((
    ...
    let changes = state
        ...
        .await
        .map_err(|e| AppError(kleos_lib::EngError::Database(e)))?;
...
async fn admin_tasks(
    ...
    require_admin(&auth)?;
    let stats = kleos_lib::jobs::get_job_stats(&state.db.conn).await?;
    to_json(stats)
}
...
async fn admin_cred_resolve(
    ...
    let service = body
        .service
        .as_deref()
        .ok_or_else(|| AppError(kleos_lib::EngError::InvalidInput("service is required".into())))?;
    let key = body
        .key
        .as_deref()
...
async fn reset_user(
    ...
    for sql in tables {
        total += state
            ...
            .await
            .map_err(kleos_lib::EngError::Database)? as i64;
    }
