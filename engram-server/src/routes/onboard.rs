...
use kleos_lib::memory::{self, search::hybrid_search, types::{SearchRequest, StoreRequest}};
...
async fn fetch_url(
    ...
    let parsed = url::Url::parse(&body.url).map_err(|_| {
        AppError(kleos_lib::EngError::InvalidInput("Invalid URL".into()))
    })?;
    ...
    let raw = resp
        .text()
        .await
        .map_err(|e| AppError(kleos_lib::EngError::Internal(format!("Read error: {}", e))))?;
