...
async fn export_handler(
    ...
    Ok(Json(serde_json::to_value(data).map_err(|e| AppError(kleos_lib::EngError::Internal(e.to_string())))?))
}
...
async fn import_handler(
    ...
    if let Some(obj) = body.as_object() {
        if obj.contains_key("memories") {
            ...
            if version.is_some() {
                return import_kleos_export(&state, auth.user_id, obj).await;
            }
    ...
    Err(AppError(kleos_lib::EngError::InvalidInput("unrecognized import format".into())))
}
...
async fn import_kleos_export(
    ...
    Ok(Json(json!({ "imported": imported, "skipped": skipped, "format": "kleos" })))
}
...
async fn get_state_handler(
    ...
    let all = kleos_lib::admin::list_state(&state.db).await?;
...
async fn delete_state_handler(
    ...
    ).await.map_err(|e| AppError(kleos_lib::EngError::Internal(e.to_string())))? as i64;
...
async fn list_preferences_handler(
    ...
    Ok(Json(serde_json::to_value(prefs).map_err(|e| AppError(kleos_lib::EngError::Internal(e.to_string())))?))
}
...
async fn get_preference_handler(
    ...
    Ok(Json(serde_json::to_value(pref).map_err(|e| AppError(kleos_lib::EngError::Internal(e.to_string())))?))
}
