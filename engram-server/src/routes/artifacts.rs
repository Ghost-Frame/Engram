...
async fn download_artifact(
    ...
    let memory_id = artifact.memory_id.ok_or_else(|| {
        AppError(kleos_lib::EngError::NotFound("Artifact has no associated memory".into()))
    })?;
    ...
    let row = rows.next().await.map_err(kleos_lib::EngError::Database)?;
