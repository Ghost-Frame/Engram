...
async fn get_prompt(
    ...
    let result = kleos_lib::prompts::generate_prompt(&state.db, format, budget, context, auth.user_id).await?;
    Ok(Json(json!({
...
async fn generate_prompt_handler(
    ...
    let result = kleos_lib::prompts::generate_prompt(&state.db, fmt, budget, &context, auth.user_id).await?;
...
async fn post_header(
    ...
    let result = kleos_lib::prompts::generate_header(&state.db, actor_model, actor_role, context, limit, auth.user_id).await?;
    Ok(Json(json!({
