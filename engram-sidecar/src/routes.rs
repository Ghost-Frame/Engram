...
async fn recall(
    ...
    // Build search request for Kleos server
    let search_req = json!({
        "query": body.query,
    ...
    let response = req.send().await.map_err(|e| {
        tracing::error!(user_id = state.user_id, error = %e, "kleos server request failed");
        (
    ...
    for obs in &observations {
        ...
        match http_req.send().await {
            Ok(response) if response.status().is_success() => {
                stored += 1;
                tracing::debug!(tool = %obs.tool_name, "observation stored via kleos server");
            }
            Ok(response) => {
                tracing::error!(
                    tool = %obs.tool_name,
                    status = %response.status(),
                    "kleos server rejected observation"
                );
            }
            Err(e) => {
                tracing::error!(
                    ...
                    user_id = state.user_id,
                    error = %e,
                    "failed to send observation to kleos server"
                );
            }
        }
    }
