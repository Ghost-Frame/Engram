...
async fn resume_session(
    ...
    let store = state.session_store.as_ref().ok_or_else(|| {
        (
            ...
            Json(json!({
                "error": "persistent session store not configured -- set KLEOS_SIDECAR_STORE_PATH",
            })),
        )
    })?;
...
// ---------------------------------------------------------------------------
//
// Sends all pending observations for `session_id` to the kleos server in a
...
pub(crate) async fn flush_pending(state: &SidecarState, session_id: &str) -> usize {
    ...
    let response = match req.send().await {
        ...
        Err(e) => {
            tracing::error!(
                ...
                user_id = state.user_id,
                error = %e,
                "batch flush: kleos server unreachable"
            );
            return 0;
        }
    };
    ...
    if !status.is_success() && status != reqwest::StatusCode::MULTI_STATUS {
        tracing::error!(
            session_id = %session_id,
            status = %status,
            "batch flush: kleos server rejected batch"
        );
        return 0;
    }
