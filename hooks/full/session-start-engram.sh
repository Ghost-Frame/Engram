...
printf 'bootstrapping\t%s\t%s\n' "$_NOW" "early" > "$STATE_DIR/kleos-ready-${PPID:-$$}" 2>/dev/null || true
printf 'global\t%s\n' "$_NOW" > "$STATE_DIR/kleos-ready-global" 2>/dev/null || true
...
# --- Clear kleos-search enforcement stamp from previous session ---
rm -f "$STATE_DIR/kleos-searched" 2>/dev/null || true
log "Cleared kleos-searched stamp"
...
if ! curl -sf --max-time 1 "http://localhost:7711/health" > /dev/null 2>&1; then
  ...
  if [ -f "$MNEMONIC_BIN" ]; then
    KLEOS_URL="${KLEOS_URL:-http://100.64.0.13:4200}" KLEOS_API_KEY="${KLEOS_API_KEY:-}" nohup node --experimental-strip-types "$MNEMONIC_BIN" >> "$LOG_DIR/mnemonic.log" 2>&1 &
    disown
