...
# Calls /gate/complete (enforces Kleos store requirement), /activity (fans out to all services).
...
SESSION_KEY="${PPID:-$$}"
STAMP_FILE="$STATE_DIR/kleos-ready-${SESSION_KEY}"
...
# Build summary from recent Kleos memories (best-effort, local)
KLEOS_CLI="$HOME_DIR/.local/bin/kleos-cli"
SUMMARY=$("$KLEOS_CLI" recall --limit 3 --json 2>/dev/null | python3 -c "
import sys, json
try:
...
# 1. Call Eidolon /gate/complete (checks Kleos store requirement)
...
" "$GATE_RESPONSE" 2>/dev/null || echo "True")
  if [ "$GATE_ALLOWED" = "False" ] || [ "$GATE_ALLOWED" = "false" ]; then
    log "Gate/complete denied -- session may not have stored to Kleos"
  fi
fi

# 2. Call Eidolon /activity task.completed (fans out to Chiasm, Axon, Broca, Kleos, Brain)
...
KLEOS_STORES=0
if [ -f "$LOG_DIR/kleos-usage.log" ]; then
  KLEOS_STORES=$(grep -c "store" "$LOG_DIR/kleos-usage.log" 2>/dev/null || echo 0)
fi
...
EIDOLON_URL_END="${EIDOLON_URL:-http://100.64.0.13:7700}"
if [ -n "$EIDOLON_KEY_END" ]; then
  curl -sf --max-time 3 "$EIDOLON_URL_END/activity" \
    -X POST \
    -H "Authorization: Bearer $EIDOLON_KEY_END" \
    -H "Content-Type: application/json" \
    -d "{\"agent\":\"claude-code\",\"action\":\"session.quality\",\"summary\":\"Session compliance score: $SCORE (turns=$TURN_COUNT stores=$KLEOS_STORES eidolon=$EIDOLON_CALLS forge=$FORGE_ACTIVE)\",\"project\":\"drift-mitigation\",\"details\":{\"score\":$SCORE,\"tags\":{\"turns\":\"$TURN_COUNT\",\"kleos_stores\":\"$KLEOS_STORES\",\"eidolon_calls\":\"$EIDOLON_CALLS\",\"forge_active\":\"$FORGE_ACTIVE\",\"drift_signals\":\"$DRIFT_SIGNALS\"}}}" \
    > /dev/null 2>&1 || true
fi
...
# Never delete kleos-ready-global: it is the cross-session sentinel that
# keeps pre-bash-guardrail from blocking new sessions when PPID-keyed stamps
# haven't been written yet.
if [ "$STAMP_FILE" != "$STATE_DIR/kleos-ready-global" ]; then
  rm -f "$STAMP_FILE" 2>/dev/null || true
fi
find "$STATE_DIR" -name 'kleos-ready-*' ! -name 'kleos-ready-global' -mmin +1440 -delete 2>/dev/null || true
