...
enum CredCommands {
    ...
    ///
    /// Example:
    ///   kleos-cli cred exec kleos claude-code --env EIDOLON_KEY -- \
...
async fn main() {
    ...
    let _otel_guard = kleos_lib::observability::init_tracing("kleos-cli", "warn,yubikey=off");
...
async fn handle_cred_command(client: &Client, cmd: &CredCommands) {
    match cmd {
        ...
        CredCommands::Exec {
            ...
            //
            //   category in {"kleos","kleos"} -> per-agent Kleos
            ...
            // easy `kleos-cli cred exec kleos <slot>` form.
            let secret = if matches!(category.as_str(), "kleos" | "kleos") {
