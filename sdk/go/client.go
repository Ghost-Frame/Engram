...
// Client is the Kleos API client.
//
...
//	result, err := c.Memory.Store(ctx, kleos.StoreRequest{Content: "a fact"})
type Client struct {
	// BaseURL is the root URL of the Kleos server (no trailing slash).
	BaseURL string
...
func (c *Client) do(ctx context.Context, method, path string, body interface{}) (*http.Response, error) {
	...
	if body != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("kleos: marshal request body: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("kleos: build request: %w", err)
	}
...
// decode reads the HTTP response body into dst, and returns an KleosError for
...
func (c *Client) decode(resp *http.Response, dst interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("kleos: read response body: %w", err)
	}
	...
	if err := json.Unmarshal(raw, dst); err != nil {
		return fmt.Errorf("kleos: decode response: %w", err)
	}
	return nil
}
