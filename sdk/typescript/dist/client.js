 ...
 * Kleos Client
 *
 * TypeScript client for the Kleos memory server API.
 */
import { KleosError } from './types.js';
const DEFAULT_TIMEOUT = 30000;
/**
 * Client for interacting with the Kleos memory server.
 *
 * @example
 * ```typescript
 ...
 *
 * // Store a memory
 * const result = await kleos.store({
...
export class KleosClient {
    ...
    async request(method, path, body) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new KleosError(errorBody?.error ?? `HTTP ${response.status}`, response.status, errorBody);
            }
            return await response.json();
        }
        catch (error) {
            clearTimeout(timeoutId);
            if (error instanceof KleosError) {
                throw error;
            }
            if (error instanceof Error) {
                if (error.name === 'AbortError') {
                    throw new KleosError('Request timeout', 408);
                }
                throw new KleosError(error.message, 0);
            }
            throw new KleosError('Unknown error', 0);
        }
    }
    // -------------------------------------------------------------------------
    // Memory Operations
    ...
    async get(id) {
        try {
            ...
            if (error instanceof KleosError && error.statusCode === 404) {
                return null;
            }
            throw error;
        }
