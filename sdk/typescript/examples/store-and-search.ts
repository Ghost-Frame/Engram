 ...
 * and searching for them using the Kleos SDK.
 *
 * Run with:
 *   npx tsx examples/store-and-search.ts
 *
 * Requires:
 *   - KLEOS_URL (defaults to http://localhost:4200)
 *   - KLEOS_API_KEY (required)
 */
...
async function main() {
  ...
  const client = new KleosClient({
    url: process.env.KLEOS_URL || 'http://localhost:4200',
    apiKey: process.env.KLEOS_API_KEY!,
  });

  if (!process.env.KLEOS_API_KEY) {
    console.error('Error: KLEOS_API_KEY environment variable required');
    process.exit(1);
  }

  console.log('Connected to Kleos server');
