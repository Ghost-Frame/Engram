...
import { KleosClient, KleosError } from '../src/index.js';

describe('KleosClient', () => {
  ...
  describe('error handling', () => {
    it('should throw KleosError on API error', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 400,
      ...
      await expect(client.store({ content: '' })).rejects.toThrow(KleosError);
    });
    ...
    it('should include status code in error', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KleosError);
        expect((error as KleosError).statusCode).toBe(401);
        expect((error as KleosError).message).toBe('Unauthorized');
      }
    });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(client.get(1)).rejects.toThrow(KleosError);
    });
  });
  ...
  describe('URL handling', () => {
    it('should handle URLs with trailing slash', async () => {
      const clientWithSlash = new KleosClient({
        url: 'http://localhost:4200/',
        apiKey: 'test',
      });
