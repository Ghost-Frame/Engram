...
#[command(about = "ETL tool to migrate Kleos from libsql to rusqlite + LanceDB")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("kleos_migrate=info".parse()?))
        .init();
