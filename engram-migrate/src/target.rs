...
use kleos_lib::db::schema_sql::{
    AUXILIARY_SCHEMA_STATEMENTS, CORE_SCHEMA_SQL, KLEOS_SERVICES_SQL,
};
...
pub async fn create(target_dir: &Path) -> Result<TargetDb> {
    ...
    let db_path = target_dir.join("kleos.db");
    ...
    // Run schema creation from kleos-lib constants
    ...
    info!("Creating Kleos services schema...");
    conn.execute_batch(KLEOS_SERVICES_SQL)?;
